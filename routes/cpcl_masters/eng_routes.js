...
router.post('/engineer/new',(req,res)=>{
    var eic_saluation = req.body.eic_saluation;
    ...
    async function getEngvalues(){
        try{
            let pool = await sql.connect(config);
             await pool.request().query("insert into cpcl_engineer_master (eic_saluation,EIC_PRNO,name,department,designation,email,mobile,user_name,password,status) values ('"+eic_saluation+"','"+eic_prno+"','"+name+"','"+department+"','"+designation+"','"+email+"','"+mobile+"','"+eic_prno+"','"+eic_prno+"','"+status+"')",(req,res)=>{
...
router.post('/update',(req, res) => {
    ...
    async function Engineupdate(){
        try{
            let pool = await sql.connect(config);
            let products = await pool.request().query(`update cpcl_engineer_master SET eic_saluation='${req.body.eic_saluation}',
